...
with open("prc_fl_stat_znl.txt") as exc_l_fi_let:
...
with open("prc_fl_stat_znl.csv","w") as execoutfile:
...
n_df_nod_ref = pd.read_csv('prc_fl_stat_znl.csv')
...
n_df_nod_ref_incoming.to_csv(r'inprocess.txt',header=None, index=None,mode='a')
n_df_nod_ref_outgoing.to_csv(r'outprocess.txt',header=None, index=None,mode='a')

with open('twowayprocess.html','w') as twowyfile:
...
with open("incoming.txt", "w") as output_file:
    for onlod_sec_nfl_l in onlod_sec_nfl_ls:
        onlod_sec_nfl_ip_list = cmpl_pat_2.findall(onlod_sec_nfl_l)
        if not onlod_sec_nfl_ip_list[1] == myip:
            output_file.write(onlod_sec_nfl_ip_list[1]+'\n')
with open("outgoing.txt", "w") as output_file:
    for onlod_sec_nfl_l in onlod_sec_nfl_ls:
        onlod_sec_nfl_ip_list = cmpl_pat_2.findall(onlod_sec_nfl_l)
        if not onlod_sec_nfl_ip_list[0] == myip:
            output_file.write(onlod_sec_nfl_ip_list[0]+'\n')

with ZipFile('download.zip','w') as dn_load_fl:
